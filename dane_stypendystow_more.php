...
<div id="page-wrapper">
	...
	if(isset($_SESSION['cloak_to'])){ ?>
		...
		//echo $rok_wnioskow;
		?>
		<h1 align="center">Lista stypendystów (<?php echo $rok_wnioskow; ?>)</h1>
...
$dane_wreczenie = $db -> query("SELECT * FROM `dane_podstawowe` WHERE dane_wniosku LIKE '%$rok_wnioskow%'");
    ...
    foreach($db->results() as $dane) {
		...
		echo "<td title=imie>".$dane->imie."</td>";
		echo "<td title=imie>".$dane->nazwisko."</td>";
		
		
		
		echo "<td><a href='dane_stypendystow_more_pokaz.php?id=$id'>Pokaż infomacje</a></td>";
