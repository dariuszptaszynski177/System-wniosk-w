            ...
            if ($login) {
              $dest = sanitizedDest('dest');
               ...
               if($twoQ->count()>0) {
                ...
                $error_message .= '<strong>Niepoprawne dane logowania</strong>. Sprawdź login i hasło';
            }
        } else{
            $error_message .= '<ul>';
...
?>
    ...
    <!--<h2 class="form-signin-heading"></i> <?=lang("SIGNIN_TITLE","");?></h2>-->
		<h2 class="form-signin-heading"></i>Zaloguj się</h2>
    <input type="hidden" name="dest" value="<?= $dest ?>" />

    <div class="form-group">
        <label for="username" >Nazwa użytkownika lub e-mail</label>
        <input  class="form-control" type="text" name="username" id="username" placeholder="Nazwa użytkownika/E-mail" required autofocus>
    </div>

    <div class="form-group">
        <label for="password">Hasło</label>
        <input type="password" class="form-control"  name="password" id="password"  placeholder="Hasło" required autocomplete="off">
    </div>
    <?php
    if($settings->recaptcha == 1){
    ?>
    ...
    <div class="form-group">
    <label for="remember">
    <input type="checkbox" name="remember" id="remember" > Zapamiętaj mnie</label>
    </div>

    <input type="hidden" name="csrf" value="<?=Token::generate(); ?>">
    <input type="hidden" name="redirect" value="<?=Input::get('redirect')?>" />
    <button class="submit  btn  btn-primary" id="next_button" type="submit"><i class="fa fa-sign-in"></i> Zaloguj</button>
...
<div class="row">
    <div class="col-xs-6"><br>
        <a class="pull-left" href='forgot_password.php'><i class="fa fa-wrench"></i> Zapomniane hasło?</a><br><br>
    </div>
    <?php if($settings->registration==1) {?>
      <div class="col-xs-6"><br>
        <a class="pull-right" href='join.php'><i class="fa fa-plus-square"></i> Zarejestruj się</a><br><br>
    </div><?php } ?>
</div>
</div>
</div>
