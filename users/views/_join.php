...
<form class="form-signup" action="<?=$form_action;?>" method="<?=$form_method;?>" id="payment-form">
        ...
        <div class="form-group">
                ...
                <?php if($settings->auto_assign_un==0) {?><label>Nazwa użytkownika:</label>&nbsp;&nbsp;<span id="usernameCheck" class="small"></span>
                <input type="text" class="form-control" id="username" name="username" placeholder="Nazwa użytkownika" value="<?php if (!$form_valid && !empty($_POST)){ echo $username;} ?>" required autofocus><?php } ?>


                <label for="fname">Imię*</label>
                <input type="text" class="form-control" id="fname" name="fname" placeholder="Imię" value="<?php if (!$form_valid && !empty($_POST)){ echo $fname;} ?>" required autofocus>

                <label for="lname">Nazwisko*</label>
                <input type="text" class="form-control" id="lname" name="lname" placeholder="Nazwisko" value="<?php if (!$form_valid && !empty($_POST)){ echo $lname;} ?>" required>
...
<?php
                ...
                $character_range = 'Ilość znaków od '.$settings->min_pw . ' do ' . $settings->max_pw;
                $character_statement = '<span id="character_range" class="gray_out_text">' . $character_range . ' znaków</span>';
...
if ($settings->req_num == 1){
                ...
                $password_match_statement = '<span id="password_match" class="gray_out_text">Hasła zgadzają się</span>';
...
?>
                ...
                <div style="display: inline-block">
                        <label for="password">Wpisz hasło* (Ilość znaków od <?=$settings->min_pw?> do <?=$settings->max_pw?> )</label>
                        <input  class="form-control" type="password" name="password" id="password" placeholder="Hasło" required autocomplete="off" aria-describedby="passwordhelp">

                        <label for="confirm">Potwierdź hasło*</label>
                        <input  type="password" id="confirm" name="confirm" class="form-control" placeholder="Potwierdź hasło" required autocomplete="off" >
                </div>
                <div style="display: inline-block; padding-left: 20px">
                        <strong>Hasło powinno...</strong><br>
...
<?php } ?>
                        ...
                        <br><br>
                        <a class="nounderline" id="password_view_control"><span class="glyphicon glyphicon-eye-open"></span> Pokaż hasło</a>
                </div>
                <br><br>

                <?php include('../usersc/scripts/additional_join_form_fields.php'); ?>

                <label for="confirm">Warunki rejestracji</label>
                ...
                <label><input type="checkbox" id="agreement_checkbox" name="agreement_checkbox"> Potwierdź warunki rejestracji</label>
        </div>
        ...
        <button class="submit btn btn-primary " type="submit" id="next_button"><i class="fa fa-plus-square"></i> Zarejestruj</button>
</form><br />
</div>
</div>
